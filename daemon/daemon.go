...
func NewDaemon(ctx context.Context, config *config.Config, pluginStore *plugin.Store) (daemon *Daemon, err error) {
	// 1、如果用户没有配置mtu则默认是1500
	setDefaultMtu(config)

	// 2、初始化registry service
	...
	// 设置resolv.conf解析文件，默认是/etc/resolv.conf
	...
	// 设置系统参数，一个是OMScoreAdj和MayDetachMounts
	if err := setupDaemonProcess(config); err != nil {
		return nil, err
	}

	// 设置tmp目录，默认就是 {data-toor}/tmp，然后设置环境变量
	...
	// 相当于将con中的GenericResources配置转移到daemon
	...
	// 设置secure computing mode，关于seccomp请看下面链接文章解释
	...
	// 判断内核知否支持AppArmor模块，来加载profile，AppArmor（Application Armor，应用程序防护）是一个 Linux 安全模块
	if err := ensureDefaultAppArmorProfile(); err != nil {
		logrus.Errorf(err.Error())
	}

	// 默认情况下创建的就是 /var/lib/docker/containers
	daemonRepo := filepath.Join(config.Root, "containers")
	if err := idtools.MkdirAllAndChown(daemonRepo, 0700, rootIDs); err != nil {
		return nil, err
	}

	// 默认情况下创建的就是 /var/lib/docker/runtimes
	...
	// 为metrics创建unix sock，并监听unix://{exec-root}/metrics.sock路径
	...
	// 根据配置设置daemon containerd log配置，如log-driver、log-opt等等
	if err := d.setupDefaultLogConfig(); err != nil {
		return nil, err
	}

	// 10、初始化graphdriver，储存相关，如，os：liunx，gd：overlay2
	...
	// 13、初始化volume service
	d.volumes, err = volumesservice.NewVolumeService(config.Root, d.PluginStore, rootIDs, d)
	if err != nil {
		return nil, err
	}

	// 得到 trustKey，路径在/etc/docker/key.json，如果存在读出来，如果不存在就创建文件然后生成trustKey写入文件
	trustKey, err := loadOrCreateTrustKey(config.TrustKeyPath)
	if err != nil {
		return nil, err
	}

	// 默认配置，相当与创建了/var/lib/docker/trust
	trustDir := filepath.Join(config.Root, "trust")
	if err := system.MkdirAll(trustDir, 0700, ""); err != nil {
		return nil, err
	}

	// 14、初始化 ReferenceStore
	...
	// 15、初始化distributionMetadataStore
	...
	if runtime.GOOS != "windows" {
		...
		// 迁移graph，应该是为了较老的版本迁移过来准备的，默认情况下{data-root}/graph是不存在的，所以不用迁移
	...
	// 如果{data-root}/containers中有内容说明启动之前有容器实例，这时候要恢复注册
...
func CreateDaemonRoot(config *config.Config) error {
	// 得到root-data，不指定的情况下默认是/var/lib/docker
	...
	// 如果用户配置了userns-remap，则根据配置解析出来一个IdentityMapping对象
	idMapping, err := setupRemappedRoot(config)
	if err != nil {
		return err
	}
