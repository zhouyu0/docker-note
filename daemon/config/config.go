...
func MergeDaemonConfigurations(flagsConfig *Config, flags *pflag.FlagSet, configFile string) (*Config, error) {
	// 解析json config file生成配置Config数据结构
	...
	// 合并json config file和flags config，最终赋值于fileConfig对象
	// 详细看meger包的属性
...
func getConflictFreeConfiguration(configFile string, flags *pflag.FlagSet) (*Config, error) {
	...
	if flags != nil {
		...
		// 以map[string]interface{}的形式解析出来json配置文件
		if err := json.NewDecoder(reader).Decode(&jsonConfig); err != nil {
			return nil, err
		}

		/* 将json文件中的第一层value还是个map[string]interface{}的设置在第一层，如:
		{
			"a": "a",
			"b": {
				"c": "c"
...
func findConfigurationConflicts(config map[string]interface{}, flags *pflag.FlagSet) error {
	// 从文件的map中找出没有在flags中注册的键值，存在unknownKeys map中，
	...
	// unknownKeys map有内容，则要排除那些在json file和命令行参数中名称不一样的键值
	// 如：在json文件中registry-mirrors是个list，而要在命令行参数中就是解析registry-mirror字段（--registry-mirror=http://xxxxx.xx.com）
	...
	// 如果现在unknownKeys map还有内容，说明真的不能解析了
	if len(unknownKeys) > 0 {
		var unknown []string
	...
	// 找出在json文件中设置又被flags解析出来的，存在conflicts中
